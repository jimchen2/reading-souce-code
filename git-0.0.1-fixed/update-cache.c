 ...
 */
#include "cache.h"
#include <arpa/inet.h>  // Add this for htonl
...
static int write_cache(int newfd, struct cache_entry **cache, int entries)
{
	...
	hdr.signature = htonl(CACHE_SIGNATURE); // Convert to big-endian
	hdr.version = htonl(1);					// Convert to big-endian
	hdr.entries = htonl(entries);			// Convert to big-endian
