...
static inline unsigned int big_endian_to_host(unsigned int val)
{
...
static int verify_hdr(struct cache_header *hdr, unsigned long size)
{
	SHA_CTX c;
	unsigned char sha1[20];
	unsigned int signature = big_endian_to_host(hdr->signature);
	unsigned int version = big_endian_to_host(hdr->version);
...
int read_cache(void)
{
	...
	active_nr = big_endian_to_host(hdr->entries);
	active_alloc = alloc_nr(active_nr);
	active_cache = calloc(active_alloc, sizeof(struct cache_entry *));

	offset = sizeof(*hdr);
	for (i = 0; i < active_nr; i++) // Use active_nr for consistency
	{
